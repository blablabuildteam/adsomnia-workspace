...
| **Domain allowlist** | `GOOGLE_ALLOWED_DOMAINS` — comma-separated (e.g. `adsomnia.com,blablabuild.com,godai.nl`). `godai.nl` is also allowed in code. |
...
| **Admins** | Emails in `LOGIN_*_EMAIL` (Adsomnia Sietse/Oleg/Jasper/Coen + Godai `sietse@godai.nl` / `jesper@godai.nl` + blablabuild Xennith/Kevin) are assigned `leadership` on Google login (create or promote) |
...
| `LOGIN_*_EMAIL` | Server | Leadership admin emails (Sietse/Oleg/Jasper/Coen/Xennith/Kevin, plus Godai aliases) — assigned `leadership` on Google login |
...
GOOGLE_ALLOWED_DOMAINS=adsomnia.com,blablabuild.com,godai.nl
```
